...
export default function CodeEditor({ questionId }: CodeEditorProps) {
  ...
  // Refs to manage polling and timeout so we can clear them reliably
  ...
  const handleRuntimeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setLanguage(linked?.language ?? "plaintext");
  };
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      // Safety timeout after 30s — clear interval and report unknown
